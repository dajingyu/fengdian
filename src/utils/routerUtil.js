...
import { getRouterByUser } from '@/api/system'
...
const constantRouterComponents = {
  ...
  //系统管理相关
  FanList: () => import('@/views/system/FanList'),

  //用户相关
  center: () => import('@/views/personal/Center'),

  404: () => import('@/views/exception/404'),

  //业务相关
  singleMonitor: () => import('@/views/monitor/singleMonitor')
}
...
export const generatorDynamicRouter = () => {
  return new Promise((resolve, reject) => {
    ...
    //如果菜单从接口输出，此代码展示 start
    // ajax
    // const loginParams = {}
    // loginParams.url = '/goEms'
    // loginParams.wfCode = 'F040'
    // getRouterByUser(loginParams).then(res => {
...
export function getRootMenu(rows) {
  ...
  for(let i=0; i<defaultRouterMap.length; i++){
    menus.push(defaultRouterMap[i])
  }
...
export function buildtree(list, arr, parentId) {
  list.forEach(item => {
    if (item.parentId === parentId) {
      ...
      if(item.url == 'PageView,overview'){  //总览菜单隐藏
        item.visible = '1'
      }
      var child = {
        title: item.name,
      ...
      buildtree(list, child.children, item.uid)
