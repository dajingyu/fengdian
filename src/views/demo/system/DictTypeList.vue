  ...
  <a-card :bordered="false">
        ...
        <a-row :gutter="48">
          ...
          <a-col :md="5" :sm="15">
            <a-form-item label="字典类型">
              <a-input placeholder="请输入" v-model="queryParam.type"/>
            </a-form-item>
          </a-col>
          <a-col :md="5" :sm="15">
            <a-form-item label="是否是系统">
              <a-select placeholder="请选择" v-model="queryParam.isSys" default-value="0">
    ...
    >
      ...
      <span slot="action" slot-scope="text, record">
        ...
        <a v-if="editEnabel" @click="dataModal(record.id, record.type)"><a-icon type="bars" />列表</a>
        <a-divider type="vertical" />
        <a v-if="removeEnable" @click="delByIds([record.id])">删除</a>
      </span>
    </s-table>
    <dict-type-modal ref="modal" @ok="handleOk" />
...
import DictTypeModal from './modules/DictTypeModal.vue'
...
export default {
  ...
  methods: {
    ...
    dataModal (dictTypdId, dictTypd) {
      this.$refs.datamodal.show(dictTypdId, dictTypd)
    },
    delByIds (ids) {
